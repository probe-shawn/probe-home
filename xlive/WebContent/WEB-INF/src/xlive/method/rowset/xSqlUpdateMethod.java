...
public class xSqlUpdateMethod extends xDefaultMethod{
	...
	public Object process() throws xMethodException{
		...
		*/
		...
		try{
			...
			command.append(table_name).append(" set ");
			NodeList params_nodelist=(NodeList)getArguments("update-fields/child::*", XPathConstants.NODESET);
			...
			if(constrain_params_nodelist.getLength() > 0){
				for(int i=0; i<constrain_params_nodelist.getLength(); ++i){
					...
					if("static".equals(data_type)){
						if(add_coma)command.append(" , ");
						add_coma=true;
						command.append(constrain_field.getNodeName()).append(" = ").append(constrain_field.getTextContent());
						continue;
					}
					/////update only in constrain fields
					value=null;
					...
					if(add_coma)command.append(" , ");
					add_coma=true;
					command.append(constrain_field.getNodeName()).append(" = ").append(" ? ");
				...
				for(int i=0; i<params_nodelist.getLength(); ++i){
					...
					command.append(field.getNodeName()).append(" = ").append(" ? ");
			...
			NodeList keys_nodelist=(NodeList)this.getArguments("key-fields/child::*", XPathConstants.NODESET);
			for(int i=0; i<keys_nodelist.getLength(); ++i){
				if(keys_nodelist.item(i).getNodeType() != Node.ELEMENT_NODE)continue;
				if(i != 0)command.append(" and ");
				Element field=(Element)keys_nodelist.item(i);
				command.append(field.getNodeName()).append(" = ").append(" ? ");
				data_type=field.getAttribute("dataType");
				value=field.getTextContent();
				String tmp=field.getAttribute("isNull");
			...
			statement=connection_instance.prepareStatement(command.toString());
			...
			//
			//this.logMessage("update command : \n"+command.toString());
			//
