...
public class uSetCompany extends aDefaultMethod{
		@Override
		public Object process(JSONObject server_jso, String method_name, JSONObject client_jso, JSONObject return_jso) throws aMethodException, JSONException {
			String why="";
			boolean valid=true;
			JSONObject company=client_jso.getJSONObject("arg").getJSONObject("company");
			String key= company.optString("key");
			String cid= company.optString("cid");
			Entity entity =null;
			...
			if(entity == null && cid !=null && cid.trim().length()>0){
				try{
					entity = ds.get(uGetCompany.generateKey(cid));
				}catch(EntityNotFoundException e){entity = null;}
			}
			if(entity == null){
				entity = new Entity(uGetCompany.generateKey(cid));
				entity.setProperty("date", new Date());
			}
			JSOToEntity(company, entity);
			ds.put(entity);	
			//
		...
		public static void JSOToEntity(JSONObject jso, Entity entity) throws JSONException{
	    	 ...
	    	 while(keys.hasNext()){
	             ...
	             if("icon".equals(key)||"allergy".equals(key)||"admins".equals(key)){
